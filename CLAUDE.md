...
**Known issues (see `BACKLOG.md`):** `cli/download.py` is broken (references config fields — e.g. `config.virtuoso` — that no longer exist post-refactor) and needs rewriting for the GraphDB backend; `cli/main.py`'s `__main__` block is not confirmed working end-to-end. `core/queries.py` has several overlapping insert/query functions flagged for consolidation. Check `BACKLOG.md` for the current TODO list before assuming a code path is exercised/working.
