...
- Package code lives under `src/kg_synth/` (installed name: `kg_synth`) — install it in editable mode (`pip install -e .`) or run scripts with `src` on `PYTHONPATH` so `from kg_synth...` imports resolve.
- Secrets/config live in `.env` (gitignored): `DATABASE_URL`.
...
The main entry point is `run_synthetic_graph_experiment` in `src/kg_synth/cli/main.py`:

```python
from pathlib import Path
from kg_synth.cli.main import run_synthetic_graph_experiment
...
**Known issues (see `backlog`):** `cli/download.py` is broken (references config fields — e.g. `config.virtuoso` — that no longer exist post-refactor) and needs rewriting for the GraphDB backend; `cli/main.py`'s `__main__` block is not confirmed working end-to-end. `core/queries.py` has several overlapping insert/query functions flagged for consolidation. Check `backlog` for the current TODO list before assuming a code path is exercised/working.
...
src/kg_synth/
  ...
  cli/
    ...
    download.py         # standalone script: paginate-download a named graph to an .nt file (currently broken)
  core/
    ...
    parsing.py           # tsv_to_nt: convert a TSV triples file to .nt format
  engine/
...
- No test suite, linting/CI pipeline, or Makefile currently exists in this repo — `ruff` and `mypy` are configured in `pyproject.toml` (strict mypy, ruff rule sets E/F/I/UP/B/N) but are not wired into any automated command; run them manually (`ruff check .`, `mypy .`) if validating changes.
